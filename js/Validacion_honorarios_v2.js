...
/*var opcion10 = document.getElementById('opcion10').value;
var opcion11 = document.getElementById('opcion11').value*/
var cuenta_errora=0;
...
for(var p=1; p<=150; p++){
	//input
	var errnombre_artista_grupo = document.getElementById('nombre_artista_grupo'+p).value;
	var estado_origen_artista_grupo = document.getElementById('estado_origen_artista_grupo'+p).value;
	var mun_origen_artista_grupo = document.getElementById('mun_origen_artista_grupo'+p).value;
	var num_tel_artista_grupo = document.getElementById('num_tel_artista_grupo'+p).value;
	var email_artista_grupo = document.getElementById('email_artista_grupo'+p).value;
	var reconoc_import_art_grupo = document.getElementById('reconoc_import_art_grupo'+p).value;
	var paginaweb_redessoc_artista_grupo = document.getElementById('paginaweb_redessoc_artista_grupo'+p).value;
	var anio_experiencia_comprobables = document.getElementById('anio_experiencia_comprobables'+p).value;
	var nombre_rep_fis_artista_grupo = document.getElementById('nombre_rep_fis_artista_grupo'+p).value;
	var num_tel_rep_fis_artista_grupo = document.getElementById('num_tel_rep_fis_artista_grupo'+p).value;
	var email_rep_fis_artista_grupo = document.getElementById('email_rep_fis_artista_grupo'+p).value;
	var nombre_presentacion = document.getElementById('nombre_presentacion'+p).value;
	var fecha_presentacion = document.getElementById('fecha_presentacion'+p).value;
	var horario = document.getElementById('horario'+p).value;
	var num_par_por_grupo = document.getElementById('num_par_por_grupo'+p).value;
	var num_Mujeres = document.getElementById('num_Mujeres'+p).value;
	var num_Hombres = document.getElementById('num_Hombres'+p).value;
	var duracion_espec_prop = document.getElementById('duracion_espec_prop'+p).value;
	var nombre_foro = document.getElementById('nombre_foro'+p).value;
	/*var estado_foro = document.getElementById('estado_foro'+p).value;
	var mun_alcaldia_foro = document.getElementById('mun_alcaldia_foro'+p).value;
	var localidad_foro = document.getElementById('localidad_foro'+p).value;*/
	var publico_dir_prop_art = document.getElementById('publico_dir_prop_art'+p).value;
	var impacto_socio_cultural_esp = document.getElementById('impacto_socio_cultural_esp'+p).value;
	var sinopsis = document.getElementById('sinopsis'+p).value;
	var links_mat_videograf_prop = document.getElementById('links_mat_videograf_prop'+p).value;
	var monto_honorarios_imp_inc_mn = document.getElementById('monto_honorarios_imp_inc_mn'+p).value;

	//select
	var confirmado_tentativo = document.getElementById('confirmado_tentativo'+p).value;
	var disciplina = document.getElementById('disciplina'+p).value;
	var categoria = document.getElementById('categoria'+p).value;
	var artista_cuenta_con_CFDI = document.getElementById('artista_cuenta_con_CFDI'+p).value;
	var rep_fis_cuenta_con_CFDI = document.getElementById('rep_fis_cuenta_con_CFDI'+p).value;
	var espacio_requerido = document.getElementById('espacio_requerido'+p).value;
	var cuenta_actualmente_con_apoyo_FONCA = document.getElementById('cuenta_actualmente_con_apoyo_FONCA'+p).value;
	var alcance_Artista = document.getElementById('alcance_Artista'+p).value;



// &&  nombre_foro!='' &&  estado_foro!='' &&  mun_alcaldia_foro!='' && localidad_foro!=''
...
fecha_presentacion!='' &&  horario!='' &&  num_par_por_grupo!='' && num_Mujeres!='' &&  num_Hombres!='' &&  duracion_espec_prop!='' &&  nombre_foro!='' &&  publico_dir_prop_art!='' &&  impacto_socio_cultural_esp!='' &&  sinopsis!='' &&  links_mat_videograf_prop!='' &&  monto_honorarios_imp_inc_mn!='' &&
confirmado_tentativo!='' && disciplina!='' && categoria!='' && artista_cuenta_con_CFDI!='' && rep_fis_cuenta_con_CFDI!='' && espacio_requerido!='' && cuenta_actualmente_con_apoyo_FONCA!=''){
		...
		console.log(opcion10);*/
		//if(opcion10=='10' || opcion11=='11'){
			if(suma_todo==value){
		...
		/*} else {
			if(suma_todo==value){
				//console.log('Bien Los montos ingresados deben son igual al monto de apoyo financiero solicitado a la Secretaría de Cultura.');
				document.getElementById("rowError1").style.display = 'none';
				cuenta_errora=0;
				cuenta_error_fina = 1
			} else {
				//console.log('Los montos "No son iguales" ingresados deben sumar lo mismo que el monto de apoyo financiero solicitado a la Secretaría de Cultura, porque no se selceccionó otro concepto.');
				document.getElementById("rowError2").style.display = 'block';
				cuenta_errora=1;
			}*/
...
fecha_presentacion!='' || horario!='' || num_par_por_grupo!='' || num_Mujeres!='' || num_Hombres!='' || duracion_espec_prop!='' || nombre_foro!='' || publico_dir_prop_art!='' || impacto_socio_cultural_esp!='' || sinopsis!='' || links_mat_videograf_prop!='' || monto_honorarios_imp_inc_mn!='' || 
confirmado_tentativo!='' || disciplina!='' || categoria!='' || artista_cuenta_con_CFDI!='' || rep_fis_cuenta_con_CFDI!='' || espacio_requerido!=''|| cuenta_actualmente_con_apoyo_FONCA!='') {
		// || estado_foro!='' || mun_alcaldia_foro!='' || localidad_foro!='' 
...
for(var i=1; i<=1; i++){
	...
	if(errnombre_artista_grupo=='' || estado_origen_artista_grupo=='' || mun_origen_artista_grupo=='' || num_tel_artista_grupo=='' || email_artista_grupo=='' || reconoc_import_art_grupo=='' || paginaweb_redessoc_artista_grupo=='' || 
alcance_Artista=='' || anio_experiencia_comprobables=='' || nombre_rep_fis_artista_grupo=='' || num_tel_rep_fis_artista_grupo=='' || email_rep_fis_artista_grupo=='' || nombre_presentacion=='' || 
fecha_presentacion=='' || horario=='' || num_par_por_grupo=='' || num_Mujeres=='' || num_Hombres=='' || duracion_espec_prop=='' || nombre_foro=='' || publico_dir_prop_art=='' || impacto_socio_cultural_esp=='' || sinopsis=='' || links_mat_videograf_prop=='' || monto_honorarios_imp_inc_mn=='' || confirmado_tentativo=='' || 
