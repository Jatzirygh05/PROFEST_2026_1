function validarEnvio(){
	...
	/* se quitará esta validación de este capo porque se va a eliminar
	...
	var meta_numero_grupos_ind_atenderEsc = document.getElementById('meta_numero_grupos_ind_atender');
	var errmeta_numero_grupos_ind_atender = document.getElementById('errmeta_numero_grupos_ind_atender');
	var errmeta_numero_grupos_ind_atenderAs = document.getElementById('errmeta_numero_grupos_ind_atenderAs');
	...
	var desarrollo_proyecto_rebrote_covidEsc	= document.getElementById('desarrollo_proyecto_rebrote_covid');
	var errdesarrollo_proyecto_rebrote_covid= document.getElementById('errdesarrollo_proyecto_rebrote_covid');
	var errdesarrollo_proyecto_rebrote_covidAs= document.getElementById('errdesarrollo_proyecto_rebrote_covidAs');
	...
	var estrategias_medios_otrosEsc = document.getElementById('estrategias_medios_otros');
...
//INICIO LUGARES
	...
	var cpforo_aAs = document.getElementById('errcpforo_aAs');
	var estadoforo_aAs = document.getElementById('errestadoforo_aAs');
	...
	/*var Linklug_aEsc = document.getElementById('Linklug_a');
	var errLinklug_a = document.getElementById('errLinklug_a');
	var errLinklug_aAs = document.getElementById('errLinklug_aAs');
*/
...
//FIN PRESUPUESTO	*/
	...
	var campos = [
		...
		meta_numero_grupos_ind_atenderEsc,
		...
		desarrollo_proyecto_rebrote_covidEsc,
...
organigrama_funciones3Esc
	...
	var mensaje = [
		...
		errmeta_numero_grupos_ind_atender,
		...
		errdesarrollo_proyecto_rebrote_covid,
	...
	var asteriscos = [
		...
		errmeta_numero_grupos_ind_atenderAs,
...
errdesarrollo_proyecto_rebrote_covidAs,
	...
	for ( i=0 ; i < campos.length ; i++) {
			...
			if (tomar_valor_a == '')
			{
            	...
            	if((campos[i].name=="telefono_fijo_op" || campos[i].name=="telefono_fijo_adm") && tomar_valor_a.length<10)
				{
...
if(comboEsc[j].name=="tipo_lugar_a" && selec[j].options[selec[j].selectedIndex].value==1){
...
} if(comboEsc[j].name=="tipo_lugar_a" && selec[j].options[selec[j].selectedIndex].value==2){
...
////////////////////////////////////////////////////////////////////////////////////
			...
			if(estrategias_medios_radioEsc.checked==true || estrategias_medios_televisionEsc.checked==true || estrategias_medios_internetEsc.checked==true 
				|| estrategias_medios_redes_socialesEsc.checked==true || estrategias_medios_prensaEsc.checked==true || estrategias_medios_folletos_postersEsc.checked==true 
				|| estrategias_medios_espectacularesEsc.checked==true || estrategias_medios_perifoneoEsc.checked==true || estrategias_medios_otrosEsc.checked==true){
					...
					rowError_estra_difusion.style.display = 'block';
					  cuenta_error++;
					
					}
		
			if(disciplina_musicaEsc.checked==true || disciplina_teatroEsc.checked==true || disciplina_danzaEsc.checked==true || disciplina_artes_visualesEsc.checked==true || 
			disciplina_gastronomiaEsc.checked==true || disciplina_literaturaEsc.checked==true){
					//console.log('alguna esta seleccionada');
					errdisciplinaAs.className = 'form-text';
					errmuestra_disc_error.style.display = 'none';
					rowErrordisciplina.style.display = 'none';	
				} else {	
				    //console.log('sin seleccionar disciplina');
					rowErrordisciplina.style.display = 'block';			
					errdisciplinaAs.className = 'form-text form-text-error';
					errmuestra_disc_error.style.display = 'block';
					cuenta_error++;					  
			}
	...
	if(cuenta_error == 0){
		...
		/*var Infor_finan_costo_monto_calc = document.getElementById('Infor_finan_costo_monto').value;
			
		var url_calc='calculo_presup.php?Infor_finan_costo_monto_calc='+Infor_finan_costo_monto_calc;
	
		hacerPeticion(url_calc);*/
