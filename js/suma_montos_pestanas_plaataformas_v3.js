//INICIO Función de los campos guardados en la tabla
  function obtenmontomoda(h, cosa){
        var monto_total_op = 0;
        var costov=eval('document.formul.costo_unitario_imp_incluidose__'+h+'__'+cosa+'.value');
        var unidadv=eval('document.formul.unidade__'+h+'__'+cosa+'.value');
       ...
       // var suma_pestanas_var=eval('document.formul.suma_pestanas_var.value');   
              if(unidadv.length==0 || costov.lenght==0){
               // console.log('entro 1')
                monto_v = eval('document.formul.monto_tota__'+h);
                monto_v.value = '';         
              } else {
                var monto_total_op = parseFloat(unidadv) * parseFloat(costov);
               
                var monto_total_renglon = eval('document.formul.monto_tota__'+h);
  ...
  //INICIO Función de los campos nuevos de la tabla
     function obtenmontoa(id, consecutivo){
        ...
        var unidadv= document.getElementById('unidade__'+id).value;
        var costov= document.getElementById('costo_unitario_imp_incluidose__'+id).value;
                    console.log(`unidadv: ${unidadv}`)
                     console.log(`costov: ${costov}`)
             
             if(unidadv.length==0 || costov.lenght==0){
               //console.log(`VACIS:`);
                var monto_v = eval('document.formul.monto_tota__'+consecutivo);
                monto_v.value = ''; 
                return;
              } else { 
                var monto_total_op = parseFloat(unidadv) * parseFloat(costov);
               // console.log(`NULTIPLICA: ${monto_total_op}`);
                ...
                if(isNaN(monto_total_op)){
                 //console.log(`UNO DE LOS DOS VALORES FALTAN:`); 
                    ...
                    // console.log(`LLENO:`);
                      var monto_total_renglon = eval('document.formul.monto_tota__'+consecutivo);
                       monto_total_renglon.value = monto_total_op.toFixed(2);
                  }  
            }
      ...
      function suma_verticala(id){
        ...
        for(var i=1;i<=cuantos;i++){
          ...
          var campov=eval ('document.formul.monto_tota__'+i+'.value');
