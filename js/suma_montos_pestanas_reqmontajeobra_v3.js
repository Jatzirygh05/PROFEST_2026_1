//INICIO Función de los campos guardados en la tabla
  function obtenmontomodc(h, cosa){
        var monto_total_op = 0;
        var costov=eval('document.formul.costo_unitario_imp_incluidosc__'+h+'__'+cosa+'.value');
        var unidadv=eval('document.formul.unidadc__'+h+'__'+cosa+'.value');
       ...
       // var suma_pestanas_var=eval('document.formul.suma_pestanas_var.value');   
              if(unidadv.length==0 || costov.lenght==0){
                console.log('entro 1')
                monto_v = eval('document.formul.monto_totc__'+h);
                monto_v.value = '';         
              } else {
                var monto_total_op = parseFloat(unidadv) * parseFloat(costov);
               
                var monto_total_renglon = eval('document.formul.monto_totc__'+h);
  ...
  //INICIO Función de los campos nuevos de la tabla
     function obtenmontoc(id, consecutivo){
      // console.log('entro obtenmonto');
        var unidadv=eval('document.formul.unidadc__'+id+'.value');
        var costov=eval('document.formul.costo_unitario_imp_incluidosc__'+id+'.value');
...
console.log(`unidadv:${unidadv}`);
              if(unidadv.length==0 || costov.lenght==0){
               console.log(`VACIS:`);
                var monto_v = eval('document.formul.monto_totc__'+consecutivo);
                monto_v.value = ''; 
                return;
              } else { 
                var monto_total_op = parseFloat(unidadv) * parseFloat(costov);
              ...
              if(isNaN(monto_total_op)){
                 //console.log(`UNO DE LOS DOS VALORES FALTAN:`); 
                     ...
                     //console.log(`LLENO:`);
                      var monto_total_renglon = eval('document.formul.monto_totc__'+consecutivo);
                       monto_total_renglon.value = monto_total_op.toFixed(2);
                  }  
               }
      } 
      function suma_verticalc(id){
        var sumaT=0;
        var cuantos = 50
        
        for(var i=1;i<=cuantos;i++){
         
          var campov=eval ('document.formul.monto_totc__'+i+'.value');
           ...
           if(campov.length==0){ 
              ...
              var obtsuma= document.formul.total_espc;
