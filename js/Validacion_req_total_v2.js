// JavaScript Document
function validarEnvio_presup(total_proyecto){
...
///////////////////////////// INICIO 8
var m1_fuente_8 = document.getElementById('total_esp_mue_inmueg').value;

var cuenta_errorh=0;
var cuenta_error_finh=0;
var cuenta_error_falta_inforh=0;
		
for(var p=1; p<=30; p++){

	var errconceptoh     = document.getElementById('conceptoh'+p).value;
	var errunidadh 	     = document.getElementById('unidadh'+p).value;
	var errcosto_unitario_imp_incluidosh = document.getElementById('costo_unitario_imp_incluidosh'+p).value;
	var errmonto_tot_imp_incluidosh = document.getElementById('monto_tot_imp_incluidosh'+p).value;
		
	if(errconceptoh!='' && errunidadh!='' && errcosto_unitario_imp_incluidosh!='' && errmonto_tot_imp_incluidosh!=''){
					
				if(m1_fuente_8==value){
					console.log('Los montos ingresados son iguales al monto de apoyo financiero solicitado a la Secretaría de Cultura.');
					document.getElementById("rowError8").style.display = 'block';
					cuenta_errorh=0;
					cuenta_error_finh = 1
					} else {
						console.log('Los montos "No son iguales" ingresados deben sumar lo mismo que el monto de apoyo financiero solicitado a la Secretaría de Cultura.');
	...
	} else if(errconceptoh!='' || errunidadh!='' || errcosto_unitario_imp_incluidosh!='' || errmonto_tot_imp_incluidosh!='') {
		console.log('Verifica tu captura ya que no tiene la información necesaria');
		document.getElementById("rowError8").style.display = 'block';
				
		cuenta_error_falta_inforh=1;
		}	
} //fin for
...
for(var i=1; i<=3; i++){
	...
	var errconceptoh	 = document.getElementById('conceptoh'+i).value;
	var errunidadh	 = document.getElementById('unidadh'+i).value;
	var errcosto_unitario_imp_incluidosh	 = document.getElementById('costo_unitario_imp_incluidosh'+i).value;
	var errmonto_tot_imp_incluidosh	 = document.getElementById('monto_tot_imp_incluidosh'+i).value;
...
///////////////////////////// FIN 9
 ...
 var cuenta_error_totales=0;
	...
	//8
	if(cuenta_errorh == 0 && cuenta_error_finh == 0 && cuenta_error_falta_inforh==0){	
		//console.log('SI se puede guardar');
		//window.document.formulb.submit();
		document.getElementById("rowBien8").style.display = 'block';
		document.getElementById("rowError8").style.display = 'none';
		$(window).scrollTop(300);
 	} else {
		//console.log('error en la captura');
		document.getElementById("rowError8").style.display = 'block';
		document.getElementById("rowBien8").style.display = 'none';
		$(window).scrollTop(300);
		cuenta_error_totales=1;	
		}
	//9
