//INICIO Función de los campos guardados en la tabla
  function obtenmontomodd(h, cosa){
        var monto_total_op = 0;
        var costov=eval('document.formul.costo_unitario_imp_incluidosd__'+h+'__'+cosa+'.value');
        var unidadv=eval('document.formul.unidadd__'+h+'__'+cosa+'.value');
      ...
      // console.log(`obtenmontomodd unidadv: ${unidadv}`) 
                  console.log(`costov: ${costov}`) 
              if(unidadv.length==0 || costov.lenght==0){
                //console.log('entro 1')
                monto_v = eval('document.formul.monto_totd__'+h);
                monto_v.value = '';         
              } else {
                var monto_total_op = parseFloat(unidadv) * parseFloat(costov);
               
                var monto_total_renglon = eval('document.formul.monto_totd__'+h);
  ...
  //INICIO Función de los campos nuevos de la tabla
     function obtenmontod(id, consecutivo){
       // console.log('entro obtenmonto');
        var unidadv=eval('document.formul.unidadd__'+id+'.value');
        var costov=eval('document.formul.costo_unitario_imp_incluidosd__'+id+'.value');
       ...
       /*var unidadv= document.getElementById('unidad__'+id).value;
        ...
        ///console.log(`unidadv:${unidadv}`);
              if(unidadv.length==0 || costov.lenght==0){
               //console.log(`VACIS:`);
                var monto_v = eval('document.formul.monto_totd__'+consecutivo);
                monto_v.value = ''; 
                return;
              } else { 
                var monto_total_op = parseFloat(unidadv) * parseFloat(costov);
              ...
              if(isNaN(monto_total_op)){
                 //console.log(`UNO DE LOS DOS VALORES FALTAN:`); 
                     ...
                     //console.log(`LLENO:`);
                      var monto_total_renglon = eval('document.formul.monto_totd__'+consecutivo);
                       monto_total_renglon.value = monto_total_op.toFixed(2);
                  }  
               }

      } 
      function suma_verticald(id){
        var sumaT=0;
        var cuantos = 50
        
        for(var i=1;i<=cuantos;i++){
         
          var campov=eval ('document.formul.monto_totd__'+i+'.value');
           ...
           if(campov.length==0){ 
              ...
              var obtsuma= document.formul.total_espd;
