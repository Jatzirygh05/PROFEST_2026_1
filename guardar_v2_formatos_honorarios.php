...
//&& $estado_foro!='' && $mun_alcaldia_foro!='' && $localidad_foro!=''
	...
	if($nombre_artista_grupo!='' && $estado_origen_artista_grupo!='' &&  $mun_origen_artista_grupo!='' &&  $num_tel_artista_grupo!='' &&  $email_artista_grupo!='' &&    
		...
		&& $num_Mujeres!='' &&  $num_Hombres!='' &&  $duracion_espec_prop!='' &&  $nombre_foro!='' && 
		...
		{
// `localidad_foro`, `estado_foro`, `municipio_alcaldia_foro`, 
// '$localidad_foro', '$estado_foro', '$mun_alcaldia_foro',
		$query = "INSERT INTO honorarios_artisticos_academicos_v2 
				...
				`nombre_presentacion`, `nombre_foro`, `fecha_presentacion`, `horario`, `num_participantes_por_grupo`, 
				...
				'$nombre_presentacion', '$nombre_foro', '$fecha_presentacion', '$horario', '$num_par_por_grupo', 
...
/*$query_sel_concep="SELECT * FROM v2_requerimientos_elegidos WHERE clave_usuario='".$cve_user."' and tipo in (11,10);";
$resultado_sel_concepto = mysql_query($query_sel_concep);
$cuantos_sel_concep=mysql_num_rows($resultado_sel_concepto);*/
		   

	if($mod_a){
		/*if($cuantos_sel_concep>0){
			//echo "REGISTROS  y si seleccionaron un concepto";
			echo "<META HTTP-EQUIV='Refresh' CONTENT='0;URL=v2_requerimientos_grupo1.php'>";

		} else {*/
			//echo "NO eligieron ninguna pestaña 10 y 11";
