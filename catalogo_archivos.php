			...
			if(url_antecedente){
			  ...
			  if (!permitida) { 
				 mierror = "Comprueba la extensión de los archivos a subir. \nSólo se pueden subir archivos con extensiones: " + extensiones_permitidas.join(); 
					...
					//submito!
					///////////////Validacion para que no permita pasar cuando un Nombre de archivo este vacio
					var id = document.getElementById('archivo_nombre').value;
					if(id==0){
						alert ("Debes seleccionar el Nombre del archivo.");
	...
	//console.log(`opcion ${opcion}`);
		   ...
		   switch(opcion){
			   case 1:
					...
					$("#archivo_109_descargar").hide();
					$("#archivo_110_descargar").hide();
				...
				case 3:
					...
					$("#archivo_proyecto_descargar").hide();
					$("#espacio").show();
					$("#archivo_3_descargar").show(); //antes show 21/02/2020
					$("#archivo_4_descargar").hide();
					$("#archivo_5_descargar").hide();
					$("#archivo_6_descargar").hide();
					$("#archivo_7_descargar").hide();
				...
				case 5:
					...
					$("#espacio").show();
					$("#archivo_3_descargar").hide();
					$("#archivo_4_descargar").show();//antes show 21/02/2020
					$("#archivo_5_descargar").hide();
					$("#archivo_6_descargar").hide();
					$("#archivo_7_descargar").hide();
					$("#archivo_8_descargar").hide();
...
<?php /*div class="row top-buffer">
    <div class="col-md-12">
	    Solicitud de apoyo para festivales culturales y artísticos PROFEST <a href="Solicitud.php" border='0' target='_new'><span class='glyphicon glyphicon-file' aria-hidden='true'></span></a>
	    <br />
	    Formato de proyecto para festivales culturales y artísticos PROFEST <a href="reporte/Formato_proyecto.php" border='0' target='_new'><span class='glyphicon glyphicon-file' aria-hidden='true'></span></a>
	</div>
</div*/?>  
			...
			<div class="modal-content">					
					<div class="modal-header">						
						...
						/*$sql = "SELECT usuarios.tipo_instancia, cat_instancias.id_instancia, cat_instancias.nombre_instancia 
									FROM usuarios,cat_instancias WHERE usuarios.clave_usuario LIKE '".$cve_user."' and cat_instancias.nombre_instancia = usuarios.tipo_instancia"; 
						*/	
							$sql = "SELECT tipo_instancia FROM usuarios WHERE clave_usuario LIKE '".$cve_user."'"; 
							$resultado = mysqli_query($con, $sql);
							$num_resultados = mysqli_num_rows($resultado);
									for ($i=0; $i <$num_resultados; $i++){
										$row = mysqli_fetch_array($resultado);
										$id_instancia	= $row["tipo_instancia"];
									}
						?>
					</div>
							...
							<select class='form-control' name='archivo_nombre' id='archivo_nombre' onchange="mostrar(this.value);">
                            	<option value="" selected="selected">Selecciona opci&oacute;n</option>
									...
									$query_doctos="Select * From nombre_formatos where id_docto Not In (Select nombre_formato From anexos 
										where clave_usuario='".$cve_user."') and id_instancia in (0,'".$id_instancia."') order by id_docto;";
													...
													$nombre_documento=$fila_doctos['nombre_documento'];
													//$tipo_formato=$fila_doctos['tipo_formato'];	
													
													$liga_archivo=$fila_doctos['liga_archivo'];	
													//$tipo_formato=$fila_doctos['tipo_formato'];					
									?>
								<option value='<?=$id_docto?>'><?php echo $nombre_documento; ?></option>	
								<?php
								}	
								$query_doctos_d="Select * From nombre_formatos where id_docto Not In (Select nombre_formato From anexos 
										where clave_usuario='".$cve_user."') and id_docto=111 order by id_docto;";
								$res_doctos_d =  mysqli_query($con,$query_doctos_d);
								$cuantos_doctos_d =  mysqli_num_rows($res_doctos_d);
								while ($fila_doctos_d=mysqli_fetch_array($res_doctos_d, MYSQLI_ASSOC)){
													$id_docto_d=$fila_doctos_d['id_docto'];
													$nombre_documento_d=$fila_doctos_d['nombre_documento'];
													//$tipo_formato_d=$fila_doctos_d['tipo_formato'];	
													
													$liga_archivo_d=$fila_doctos_d['liga_archivo'];	
													//$tipo_formato_d=$fila_doctos_d['tipo_formato'];
								...
								if($Info_financiera_categoria=="D"){
									   ...
									   $tipo_formato='';
								?>
								<option value='<?=$id_docto_d?>'><?php echo $nombre_documento_d; ?></option>
								<?php 
									}
								}
								?>
                 ...
                 <div class="row">
                        ...
                        <label>Descarga formato*:</label>
                        <br />
                        <a href="reporte/Formato_proyecto.php" target="_new">Formato de Proyecto Cultural PROFEST</a>
                        </div>  
                        
                        <div id="archivo_3_descargar" hidden="hidden">
                        <label>Descarga formato*:</label>
                        <!--se ocupaba para 2022 y se quito para 2023 el 25012023 PRESUPUESTO_PROGRAMACION.php-->
                        <a href="reporte/Presupuesto_detallado.php" target="blank">Formato de Presupuesto PROFEST</a>
                        </div>
                        
                        <div id="archivo_4_descargar" hidden="hidden">
                        <label>Descarga formato*:</label>
                        <br />
                        <a href="reporte/dompdf/semblanza_v2.php" target="_parent">Formato de semblanza artística PROFEST</a>
                        </div>
                       ...
                       <a href="formatos_para_descarga_general/Modelo_Carta_compromiso_financiamiento.docx" target="_blank">Modelo de Carta compromiso de financiamiento(en caso de que sea expedida por la institución postulante)</a>
                        ...
                        <label>Descarga formato*:</label>
                        <br />
                         <a href="formatos_para_descarga_general/Modelo_Carta_nogestion_otrosrecursos_federales.docx" target="_blank">Modelo Carta de no gestión de otros recursos federales</a>
                        </div>  
                        ...
                        <div id="archivo_110_descargar" hidden="hidden">
                        <label>Descarga formato*:</label>
                        <br />
                        <a href="formatos_para_descarga_general/Modelo_Carta_compromiso_entrega_de_programacion.docx" target="_blank">Modelo Carta compromiso de entrega de programación confirmada</a>
                        </div> 
                        
                        <div id="archivo_109_descargar" hidden="hidden">
                        <label>Descarga formato*:</label>
                        <br />
                        <a href="formatos_para_descarga_general/Modelo_Carta_cumplimiento_al_marco_legal_INM.docx" target="_blank">Modelo Carta cumplimiento al marco legal INM</a>
                        </div> 
