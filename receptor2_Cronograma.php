...
$cuantos_cronograma=(isset($_GET['cuantos_cronograma'])?$_GET['cuantos_cronograma']:NULL);

mysql_select_db($database_automaa, $automaa);

for($i=1;$i<=$cuantos_cronograma;$i++){
...
$cuantos_id=mysql_num_rows($row1);
			...
			if($cuantos_id==1){
				//echo "----MODIFICA REGISTRO----";
					$q_modifica="UPDATE `cronograma_acciones_ejecucion_festival` SET `$variable` = '".$valor."', fecha_hora_registro=NOW()
