function validarEnvio(){
    ...
    */
	...
	var m2_fuente = Infor_finan_apoyo_montoEsc.value;
...
for(var p=1; p<=50; p++){
	...
	if(errConcepto_gasto!='' && errFuente_finan!='' && errMonto_unidad!='' && errPorcentaje!=''){
		...
		alert ('Verifica tu captura ya que no tiene la información necesaria 4');
...
///(FIN) 20012023 variables para validacion de la pestaña PRESUPUESTO	
	var cuenta_error_presupesto = 0;
	 ...
	 if(cuenta_error == 0){
		...
		if(total_esp_tra==total_2 && pres_anual_tot_2010==Infor_finan_costo_monto ){
